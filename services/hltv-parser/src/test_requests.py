#!/usr/bin/env python3
"""
Test script for HLTV Parser using requests - Testing with Vitality team
"""
...
from hltv_parser_requests import HLTVParserRequests
...
def test_vitality_requests():
    """Тестирование парсера на команде Vitality с requests"""
    try:
        parser = HLTVParserRequests()
        
        logger.info("=" * 60)
        logger.info("ТЕСТИРОВАНИЕ HLTV PARSER (REQUESTS) - КОМАНДА VITALITY")
        logger.info("=" * 60)
...
def main():
    """Основная функция"""
    try:
        success = test_vitality_requests()
        if success:
